...
router.post('/', function(req, res) {
    ...
    const attendeesList = JSON.parse(req.body.attendeesList);
    ...
    (async () => {
      ...
      const insertAttendeeInfo = 'insert into attendee_info(meeting_id, datetime_start, attendee_name, is_host) values($1, $2, $3, $4)'

      try {
          await client.query(insertMeetingInfo, meetingInfo)
          atteendeesListInfo.forEach(attendee => {
            (async () => {
              const attendeeInfo = [meeting_id, datetime_start, attendee.attendee_name, attendee.is_host]
